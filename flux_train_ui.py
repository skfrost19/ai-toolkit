...
def load_captioning(uploaded_files, concept_sentence):
    ...
    updates.append(
        gr.update(
            placeholder=f"A portrait of person in a bustling cafe {concept_sentence}",
            value=f"A person in a bustling cafe {concept_sentence}",
        )
    )
    updates.append(
        gr.update(
            placeholder=f"A mountainous landscape in the style of {concept_sentence}"
        )
    )
...
def create_dataset(*inputs):
    ...
    destination_folder = str(f"datasets/{unique_uuid}")
    if not os.path.exists(destination_folder):
        os.makedirs(destination_folder)

    for index, image in enumerate(images):
        shutil.copy(image, destination_folder)
    return destination_folder, unique_uuid
...
def start_training(
    ...
    if not lora_name:
        raise gr.Error(
            "You forgot to insert your LoRA name! This name has to be unique."
        )
    try:
        if (
            whoami()["auth"]["accessToken"]["role"] == "write"
            or "repo.write"
            in whoami()["auth"]["accessToken"]["fineGrained"]["scoped"][0][
                "permissions"
            ]
        ):
            gr.Info(
                f"Starting training locally {whoami()['name']}. Your LoRA will be available locally and in Hugging Face after it finishes."
            )
        else:
            push_to_hub = False
            gr.Warning(
    ...
    with open("config/modal_lora_config.yaml", "r") as f:
    ...
    if not push_to_hub:
        ...
        config["config"]["process"][0]["save"]["hf_repo_id"] = (
            f"{username}/{slugged_lora_name}"
        )
    ...
    if model_to_train == "schnell":
        config["config"]["process"][0]["model"]["name_or_path"] = (
    ...
    if use_more_advanced_options:
        ...
        config["config"]["process"][0] = recursive_update(
            config["config"]["process"][0], more_advanced_options_dict
        )
        print(config)
    ...
    config_path = f"tmp/{unique_uuid}-{slugged_lora_name}.yaml"
...
""",
        ...
        with gr.Accordion("Advanced options", open=False):
            steps = gr.Number(
                label="Steps", value=1000, minimum=1, maximum=10000, step=1
            )
            lr = gr.Number(
                label="Learning Rate", value=4e-4, minimum=1e-6, maximum=1e-3, step=1e-6
            )
            rank = gr.Number(
                label="LoRA Rank", value=16, minimum=4, maximum=128, step=4
            )
            model_to_train = gr.Radio(
                ["dev", "schnell"], value="dev", label="Model to train"
            )
            low_vram = gr.Checkbox(label="Low VRAM", value=False)
            with gr.Accordion("Even more advanced options", open=False):
                use_more_advanced_options = gr.Checkbox(
                    label="Use more advanced options", value=False
                )
    ...
    start.click(
        ...
        outputs=[dataset_folder, unique_uuid],
    ).then(
        fn=start_training,
        inputs=[
            lora_name,
